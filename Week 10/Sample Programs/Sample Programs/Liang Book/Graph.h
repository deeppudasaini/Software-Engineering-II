...
private:
  ...
  void dfs(int v, vector<int> &parent,
    vector<int> &searchOrders, vector<bool> &isVisited);
};
...
template <typename T>
vector<int> Graph<T>::getNeighbors(int v)
{
  return neighbors[v];
}
